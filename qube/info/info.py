...
from qube.common.suggest_similar_commands import load_available_handles
...
class TemplateInfo:
    ...
    def handle_domain_or_language_only(self, handle: str, available_templates: dict) -> None:
        ...
        # if handle exists as language in qube output its available templates and exit with zero status
    ...
    def output_table(self, templates_to_print: list, handle: str) -> None:
        ...
        table = Table(title=f'[bold]Info on qube´s {handle}', title_style="blue", header_style=Style(color="blue", bold=True), box=HEAVY_HEAD)
    ...
    def print_console_output(self, handle) -> None:
        ...
        if self.most_sim:
            ...
            if len(self.most_sim) == 1 and self.action == 'use':
                print(f'[bold red]Unknown handle \'{handle}\'. See [green]qube list [red]for all valids handles')
                ...
                nl = '\n'
                print(f'[green red]Unknown handle \'{handle}\'. See [green] qube list [red]for all valid handles.' +
    ...
    def print_suggestion(self, handle: str, domain_handle: list) -> None:
        ...
        """
        print(f'[bold red]Unknown handle \'{handle}\'. See [green] qube list [red] for all valid handles.\n')
    ...
    def load_available_languages(self, ls: list) -> set:
        """
        Load all available languages qube supports.
