...
from qube.common.suggest_similar_commands import MAIN_COMMANDS
...
class HelpErrorHandling(click.Group):
    ...
    def format_help(self, ctx, formatter):
        """
        Call format_help function with qubes customized functions.
        """
    ...
    def format_usage(self, ctx, formatter):
        ...
        """
        formatter.write_text(f'{self.get_rich_value("Usage:")} qube {" ".join(super().collect_usage_pieces(ctx))}')
    ...
    def format_commands(self, ctx, formatter):
        ...
        with formatter.section(self.get_rich_value("Commands for qube projects")):
            formatter.write_text(
        ...
        with formatter.section(self.get_rich_value("Examples")):
            formatter.write_text("$ qube create")
            formatter.write_text("$ qube bump-version 1.0.0 .")
            formatter.write_text("$ qube config all")
            formatter.write_text("$ qube info python")

        with formatter.section(self.get_rich_value("Learn more")):
            formatter.write_text("Use qube <command> --help for more information about a command. You may also want to take a look at our docs at "
                                 "https://qube.readthedocs.io/.")
...
def print_project_version(ctx, param, value) -> None:
    ...
    try:
        ...
        ctx.exit()
    # currently, its only possible to get project version from top level project dir where the qube.cfg file is
    except NoSectionError:
        ctx.fail(click.style('Unable to read from qube.cfg file.\nMake sure your current working directory has a qube.cfg file '
...
class CustomHelpSubcommand(click.Command):
    ...
    def format_help(self, ctx, formatter):
        ...
        Use the overwritten format functions this class provides to output help for each subcommand qube provides.
        """
        formatter.width = 120
        self.format_usage(ctx, formatter)
    ...
    def format_usage(self, ctx, formatter):
        ...
        """
        formatter.write_text(f'{self.get_rich_value("Usage: ")}qube {self.name} {" ".join(self.collect_usage_pieces(ctx))}')
