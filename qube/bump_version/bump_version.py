...
class VersionBumper:
    """
    Responsible for bumping the version across a qube project
    """

    def __init__(self, project_dir, downgrade):
        try:
            self.parser = ConfigParser()
            self.parser.read(f'{project_dir}/qube.cfg')
            ...
            print(
                f'[bold red]No qube.cfg file was found at [bold blue]{project_dir} [bold red]or your qube.cfg file missing required '
    ...
    def bump_template_version(self, new_version: str, project_dir: Path) -> None:
        ...
        ct_cfg_path = f'{str(project_dir)}/qube.cfg' if str(project_dir).startswith(str(Path.cwd())) else \
            f'{str(Path.cwd())}/{project_dir}/qube.cfg'
        ...
        for section in sections:
            for file, path in self.parser.items(section):
                ...
                if not not_changed:
                    path_changed = file_path if file_path.startswith(
                        str(Path.cwd())) else f'{str(Path.cwd())}/{file_path}'
        ...
        with open(f'{project_dir}/qube.cfg', 'w') as configfile:
                ...
                for line in old_file:
                    ...
                    if ('<<QUBE_NO_BUMP>>' not in line and not section == 'bumpversion_files_blacklisted') or '<<QUBE_FORCE_BUMP>>' in line:
                        ...
                        if tmp != line:
                            ...
                            print(f'[bold red]- {line.strip().replace("<!-- <<QUBE_FORCE_BUMP>> -->", "")}\n'
                                  + f'[bold green]+ {tmp.strip().replace("<!-- <<QUBE_FORCE_BUMP>> -->", "")}')
                            print()
                    else:
                        new_file.write(line)
    ...
    def can_run_bump_version(self, new_version: str, project_dir: str) -> bool:
        ...
        1.) The new version number matches the format like 1.1.0 or 1.1.0-SNAPSHOT required by qube versions
        2.) The new version is greater than the current one
        3.) The project is a qube project (this is already checked when creating the "bumper" object
    ...
    def check_bump_range(self, current_version: str, new_version: str) -> bool:
        ...
        # minor update like bumping from 1.8.5 to 1.9.0
            ...
            return new_v_split[0] == cur_v_split[0] and new_v_split[2] == '0' and (
                    int(new_v_split[1]) - int(cur_v_split[1]) == 1)

        # x-minor update like bumping from 1.8.5 to 1.8.6
        elif new_v_split[2] != cur_v_split[2]:
            return new_v_split[0] == cur_v_split[0] and new_v_split[1] == cur_v_split[1] and (
                    int(new_v_split[2]) - int(cur_v_split[2]) == 1)
    ...
    def lint_before_bump(self) -> None:
        ...
        if len(changelog_linter.failed) > 0 or len(changelog_linter.warned) > 0:
            ...
            print(
                ...
                'undefined behavior!')
            if not qube_questionary_or_dot_qube(function='confirm',
    ...
    def add_changelog_section(self, path: Path, new_version: str) -> None:
        ...
        if self.downgrade_mode:
            ...
            if self.CURRENT_VERSION.endswith('-SNAPSHOT'):
                ...
                self.insert_latest_version_section(old_changelog_file=f'{self.top_level_dir}/CHANGELOG.rst',
                                                   section=section)
