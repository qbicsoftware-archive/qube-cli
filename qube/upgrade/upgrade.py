...
class UpgradeCommand:
    """
    Responsible for checking for newer versions qube and upgrading it if required.
    """

    @staticmethod
    def check_upgrade_qube() -> None:
        """
        ...
        if not UpgradeCommand.check_qube_latest():
            if qube_questionary_or_dot_qube(function='confirm',
                                            ...
                                            default='y'):
                UpgradeCommand.upgrade_qube()

    @classmethod
    def check_qube_latest(cls) -> bool:
        """
        Checks whether the locally installed version of qube is the latest available on PyPi.

        :return: True if locally version is the latest or PyPI is inaccessible, false otherwise
        """
        latest_local_version = qube.__version__
        try:
            ...
            req = urllib.request.Request('https://pypi.org/pypi/qube/json')  # nosec
            ...
            print(
                '[bold red]Unable to contact PyPI to check for the latest qube version. Do you have an internet connection?')
        ...
        if parse_version(latest_local_version) > parse_version(latest_pypi_version):
            print(f'[bold yellow]Installed version {latest_local_version} of qube is newer than the latest release {latest_pypi_version}!'
            ...
            print(f'[bold red]Installed version {latest_local_version} of qube is outdated. Newest version is {latest_pypi_version}!')
            return False

    @classmethod
    def upgrade_qube(cls) -> None:
        """
        Calls pip as a subprocess with the --upgrade flag to upgrade qube to the latest version.
        """
        if not UpgradeCommand.is_pip_accessible():
            sys.exit(1)
        try:
            check_call([sys.executable, '-m', 'pip', 'install', '--upgrade', 'qube'])
        except Exception as e:
            print('[bold red]Unable to upgrade qube')
