...
def load_qube_template_version(handle: str, yaml_path: str) -> str:
    """
...
def load_project_template_version_and_handle(project_dir: Path) -> (str, str):
    """
    Load the template version when the user synced its project with qube the last time.
    If no sync has been done so far, its the version of the qube template the user created the project initially with.
    ...
    :return: The version number of the qube template when the user created the project and the projects template handle.
    """
    project_dir = str(project_dir)
    try:
        ct_meta = load_yaml_file(f'{project_dir}/.qube.yml')
        # split the template version at first space to omit the qube bump-version tag and return it and the the handle
        return ct_meta['template_version'].split(" ", 1)[0], ct_meta['template_handle']
    except FileNotFoundError:
        print(f'[bold red]No .qube.yml found at {project_dir}. Is this a qube project?')
        sys.exit(1)
